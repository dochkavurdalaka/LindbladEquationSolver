...
int main() {
    ...
    double* r_matrix_fast = GenerateMatrixR(l_coeff, l_coeff_conjugate, &f_tensor, &z_tensor, N);

    double* r_matrix_slow = (double*)mkl_malloc(M * M * sizeof(double), 64);
    ...
    for (int s = 0; s < M; ++s) {
        for (int n = 0; n < M; ++n) {
            ...
            r_matrix_slow[s * M + n] = -0.25 * sum_jkl;
        }
    }


    std::cout << Check(r_matrix_fast, r_matrix_slow, M) << "\n";

    // Освобождение памяти
    mkl_free(r_matrix_fast);
    mkl_free(r_matrix_slow);
