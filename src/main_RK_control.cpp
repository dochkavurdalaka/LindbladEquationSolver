...
int main() {
    ...
    double* v_full = (double*)mkl_malloc(M * sizeof(double), 64);
    double* v_rep = (double*)mkl_malloc(M * sizeof(double), 64);

    cblas_dcopy(M, v, 1, v_full, 1);
    cblas_dcopy(M, v, 1, v_rep, 1);

    for (int step_num = 0; step_num < MaxStepNum; ++step_num) {
        ...
        while (not cycle_completed) {
            {
                ...
                // Считаем v_full: v_full = v + (h/6) * (k1 + 2k2 + 2k3 + k4)
                cblas_dcopy(M, k1, 1, v_temp, 1);       // v_temp = k1
                cblas_daxpy(M, 2.0, k2, 1, v_temp, 1);  // v_temp = k1 + 2*k2
                cblas_daxpy(M, 2.0, k3, 1, v_temp, 1);  // v_temp = k1 + 2*k2 + 2*k3
                cblas_daxpy(M, 1.0, k4, 1, v_temp, 1);  // v_temp = k1 + 2*k2 + 2*k3 + k4

                cblas_dcopy(M, v, 1, v_full, 1);                // v_full = v
                cblas_daxpy(M, h / 6.0, v_temp, 1, v_full, 1);  // v_full = v_full + (h/6)*v_temp
...

            {
                ...
                // k2 = f(t + h/2, v + h/2 * k1)
                cblas_dcopy(M, v, 1, v_temp, 1);                 // v_temp = v
                cblas_daxpy(M, 0.5 * h_half, k1, 1, v_temp, 1);  // v_temp = v + 0.5*h*k1
                calculate_f(t + 0.5 * h_half, v_temp, k2, q_matrix, r_matrix, k_vector, workspace,
                            M);

                // k3 = f(t + h/2, v + h/2 * k2)
                cblas_dcopy(M, v, 1, v_temp, 1);                 // v_temp = v
                cblas_daxpy(M, 0.5 * h_half, k2, 1, v_temp, 1);  // v_temp = v + 0.5*h*k2
                calculate_f(t + 0.5 * h_half, v_temp, k3, q_matrix, r_matrix, k_vector, workspace,
                            M);

                // k4 = f(t + h, v + h * k3)
                cblas_dcopy(M, v, 1, v_temp, 1);           // v_temp = v
                cblas_daxpy(M, h_half, k3, 1, v_temp, 1);  // v_temp = v + h*k3
                calculate_f(t + h_half, v_temp, k4, q_matrix, r_matrix, k_vector, workspace, M);

                // Обновляем v_rep: v_rep = v + (h/6) * (k1 + 2k2 + 2k3 + k4)
                cblas_dcopy(M, k1, 1, v_temp, 1);       // v_temp = k1
                cblas_daxpy(M, 2.0, k2, 1, v_temp, 1);  // v_temp = k1 + 2*k2
                cblas_daxpy(M, 2.0, k3, 1, v_temp, 1);  // v_temp = k1 + 2*k2 + 2*k3
                cblas_daxpy(M, 1.0, k4, 1, v_temp, 1);  // v_temp = k1 + 2*k2 + 2*k3 + k4

                cblas_dcopy(M, v, 1, v_rep, 1);                     // v_rep = v
                cblas_daxpy(M, h_half / 6.0, v_temp, 1, v_rep, 1);  // v_rep = v_rep + (h/6)*v_temp

                // k1 = f(t, v)
                calculate_f(t + h_half, v_rep, k1, q_matrix, r_matrix, k_vector, workspace, M);

                // k2 = f(t + h/2, v + h/2 * k1)
                cblas_dcopy(M, v_rep, 1, v_temp, 1);             // v_temp = v_rep
                cblas_daxpy(M, 0.5 * h_half, k1, 1, v_temp, 1);  // v_temp = v_rep + 0.5*h*k1
                calculate_f(t + h_half, v_temp, k2, q_matrix, r_matrix, k_vector, workspace, M);

                // k3 = f(t + h/2, v + h/2 * k2)
                cblas_dcopy(M, v_rep, 1, v_temp, 1);             // v_temp = v_rep
                cblas_daxpy(M, 0.5 * h_half, k2, 1, v_temp, 1);  // v_temp = v_rep + 0.5*h*k2
                calculate_f(t + h_half, v_temp, k3, q_matrix, r_matrix, k_vector, workspace, M);

                // k4 = f(t + h, v + h * k3)
                cblas_dcopy(M, v_rep, 1, v_temp, 1);       // v_temp = v_rep
                cblas_daxpy(M, h_half, k3, 1, v_temp, 1);  // v_temp = v_rep + h*k3
                calculate_f(t + 2 * h_half, v_temp, k4, q_matrix, r_matrix, k_vector, workspace, M);
                ...
                cblas_daxpy(M, h_half / 6.0, v_temp, 1, v_full,
                            1);  // v_full = v_full + (h/6)*v_temp

                // print_vector("v", t, v, M);
                // t += h;
            }

            cblas_dcopy(M, v_full, 1, v_temp, 1);
            cblas_daxpy(M, -1.0, v_rep, 1, v_temp, 1);  // diff = x - y
            double S = cblas_dnrm2(M, v_temp, 1);

            if ((eps / 32.0) <= S and S <= eps) {
                t = t + h;
                // вот этот момент явно можно оптимизировать
                cblas_dcopy(M, v_full, 1, v, 1);
        ...
        if (t < t_end) {
            print_vector("v", t, v, M);
            std::cout << std::format("h = {}, C1 = {}, C2 = {}\n", h, C1, C2) << std::endl;
        } else if (t >= t_end) {
            double t_prev = t - h;

            double h_last = t_end - t_prev;

            // k2 = f(t + h/2, v + h/2 * k1)
            cblas_dcopy(M, v, 1, v_temp, 1);            // v_temp = v
            cblas_daxpy(M, 0.5 * h_last, k1, 1, v_temp, 1);  // v_temp = v + 0.5*h*k1
            calculate_f(t + 0.5 * h_last, v_temp, k2, q_matrix, r_matrix, k_vector, workspace, M);

            // k3 = f(t + h/2, v + h/2 * k2)
            cblas_dcopy(M, v, 1, v_temp, 1);            // v_temp = v
            cblas_daxpy(M, 0.5 * h_last, k2, 1, v_temp, 1);  // v_temp = v + 0.5*h*k2
            calculate_f(t + 0.5 * h_last, v_temp, k3, q_matrix, r_matrix, k_vector, workspace, M);

            // k4 = f(t + h, v + h * k3)
            cblas_dcopy(M, v, 1, v_temp, 1);      // v_temp = v
            cblas_daxpy(M, h_last, k3, 1, v_temp, 1);  // v_temp = v + h*k3
            calculate_f(t + h_last, v_temp, k4, q_matrix, r_matrix, k_vector, workspace, M);
            ...
            cblas_dcopy(M, v, 1, v_full, 1);                // v_full = v
            cblas_daxpy(M, h_last / 6.0, v_temp, 1, v_full, 1);  // v_full = v_full + (h/6)*v_temp

            print_vector("v", t_end, v_full, M);
            std::cout << std::format("h = {}, C1 = {}, C2 = {}\n", h, C1, C2) << std::endl;
            break;
        }
    }

    // MKL_Complex16* matrix_rho = (MKL_Complex16*)mkl_malloc(N * N * sizeof(MKL_Complex16),
    // 64); memset(matrix_rho, 0, N * N * sizeof(MKL_Complex16));
